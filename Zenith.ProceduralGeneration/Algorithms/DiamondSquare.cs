	...
	public class DiamondSquare
	{
		...
		private double[,] DiamondSquareAlgorithm()
		{
				...
				sideLength /= 2, h /= 2.0)
			{
				...
				//or distance from diamond centre to one corner
				...
				for (var x = 0; x < DATA_SIZE - 1; x += sideLength)
				{
					for (var y = 0; y < DATA_SIZE - 1; y += sideLength)
					{
						...
						//centre is average plus random offset
				...
				for (var x = 0; x < DATA_SIZE - 1; x += halfSide)
				{
					...
					for (var y = (x + halfSide) % sideLength; y < DATA_SIZE - 1; y += sideLength)
					{
						...
						var avg =
							data[(x - halfSide + DATA_SIZE) % DATA_SIZE, y] + //left of centre
							data[(x + halfSide) % DATA_SIZE, y] + //right of centre
							data[x, (y + halfSide) % DATA_SIZE] + //below centre
							data[x, (y - halfSide + DATA_SIZE) % DATA_SIZE]; //above centre
						avg /= 4.0;
						...
						//update value for centre of diamond
						data[x, y] = avg;
