	...
	public static class ColourGenerator
	{
		...
		public static HashSet<T> GenerateColours_Uniform<T>(int pixelCount, DomainBound3<float> domainBounds) where T : IVector3<float>, new()
		{
			if (typeof(T) == typeof(ColourRGB))
			{
				...
				var totalValidColours = (int)(domainBounds.X.Size * 256) * (int)(domainBounds.Y.Size * 256) * (int)(domainBounds.Z.Size * 256); // do not factor the 255 out - we must multiply in each dimension individually - combining dimensions mayb produce off-by-one errors
